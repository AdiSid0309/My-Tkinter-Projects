...
Tops=Frame(root,width=700,height=10,bd=14,relief='ridge')
Tops.pack(side=TOP)
Bottom=Frame(root,width=260,height=10,bd=14,relief='ridge')
...
def SubmitData1():
    if  acc_no.get() == "" or c_name.get() == "" or c_no.get() == "" or c_add.get() == "" :
        ...
        try:
            ...
            messagebox.showerror("Exception is", error.args)
        cursor.execute("SELECT * FROM `cust_det` ORDER BY `c_name` ASC")
...
def Database1():
    ...
    for data1 in fetch1:
        tree1.insert('', 'end', values=(data1),tags = ('odd',))
    cursor.close()
    conn.close()
...
def OnSelected2(event):
    ...
    c_no.set("")
    c_add.set("")
    acc_no.set(selecteditem[0])
    c_name.set(selecteditem[1])
    c_no.set(selecteditem[2])
    c_add.set(selecteditem[3])
    UpdateWindow = Toplevel()
    UpdateWindow.title("Contact List")
    width = 300
    height = 300
...
#-----------------------------------
style = ttk.Style()
style.configure("mystyle.tree1.column", highlightthickness=5, bd=5, font=('Times New Roman', 10), foreground="red") # Modify the font of the body
style.configure("mystyle.Treeview.Heading", font=('Times New Roman', 13,'bold'), foreground="red") # Modify the font of the headings

#----------------------Database Frame_1-------
Top1 = Frame(Tops, width=800, bd=1, relief=SOLID)
Top1.pack(side=TOP)
TableMargin1 = Frame(Tops, width=800)
...
tree1 = ttk.Treeview(TableMargin1,style="mystyle.Treeview" ,columns=("Acc_no", "Acc_Type", "Bal"), height=10, selectmode="extended", yscrollcommand=scrollbary1.set, xscrollcommand=scrollbarx1.set)
...
tree1.column('#1', stretch=NO, minwidth=0, width=90)
tree1.column('#2', stretch=NO, minwidth=0, width=90)
...
tree2 = ttk.Treeview(TableMargin2,style="mystyle.Treeview", columns=("Acc_no", "C_Name", "C_No", "C_Add"), height=80, selectmode="extended", yscrollcommand=scrollbary2.set, xscrollcommand=scrollbarx2.set)
...
tree2.column('#3', stretch=NO, minwidth=0, width=80)
tree2.column('#4', stretch=NO, minwidth=0, width=70)
tree2.pack()
